...
export const signUpAction = async (formData) => {
  ...
  if (error) {
    ...
    // Add user to MongoDB User table
    try {
      await fetch(`${origin}/api/users`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email }), // add more fields if needed
      });
    } catch (e) {
      // Optionally handle error, but don't block sign up
      console.error("Failed to add user to User table", e);
    }
